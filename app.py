...
def handle_text(message):
    ...
    if text == "👧 Luna":
        bot.send_message(message.chat.id, "👧 Saya *Luna*, AI comel bantu anda daftar dan claim Worldcoin ✨\nSila tanya apa-apa tentang claim untuk mula.", parse_mode="Markdown"); return
    if text == "🧕 Azura":
        bot.send_message(message.chat.id, "🧕 Saya *Azura*, AI profesional untuk urus Hata Wallet & pindah Worldcoin 💼\nSila tanya saya jika ingin tahu tentang transfer.", parse_mode="Markdown"); return
    if text == "👮 Pika":
        bot.send_message(message.chat.id, "👮 Saya *Pika*, AI tegas untuk bantu cashout WLD ke bank Malaysia 💸\nTanya sahaja kalau nak keluarkan duit.", parse_mode="Markdown"); return
    if text == "👩‍🏫 Sophia":
        bot.send_message(message.chat.id, "👩‍🏫 Saya *Sophia*, penasihat kripto global 🌐\nBoleh semak harga pasaran kripto terkini dengan saya.", parse_mode="Markdown"); return
    if text == "💰 Claim":
        bot.send_message(message.chat.id, f"🪪 Untuk claim Worldcoin, sila daftar di sini:\n{REFERRAL_LINK}"); return
    if text == "💸 Cashout":
        bot.send_message(message.chat.id, f"💸 Untuk cashout, pindahkan WLD ke wallet Hata Malaysia:\n{HATA_WALLET_LINK}"); return
    if text == "📢 Channel":
        bot.send_message(message.chat.id, f"📢 Sertai channel promosi kami di sini:\n{CHANNEL_LINK}"); return
    if text == "👩‍💻 Admin":
        bot.send_message(message.chat.id, f"📬 Hubungi admin jika ada soalan:\n{ADMIN_LINK}"); return
...
def handle_photo(message):
    try:
        ...
        logger.info(f"[LogLang] Image caption translated: {caption_ms}")
