import os import telebot import requests import speech_recognition as sr from gtts import gTTS from pydub import AudioSegment from flask import Flask from ocr_utils import extract_text_from_image
...
def get_ai_reply(user_input): headers = { "Authorization": f"Bearer {OPENROUTER_API_KEY}", "Content-Type": "application/json" } data = { "model": "openrouter/cypher-alpha:free", "messages": [ {"role": "system", "content": "Kamu pembantu AI comel dan mesra, bantu pengguna claim dan cashout Worldcoin ke wallet Malaysia. Tambah emoji bila perlu."}, {"role": "user", "content": user_input} ] } try: res = requests.post("https://openrouter.ai/api/v1/chat/completions", headers=headers, json=data) return res.json()['choices'][0]['message']['content'] except: return "⚠️ Maaf, AI gagal menjawab. Sila cuba sebentar lagi."

def send_text_and_voice(chat_id, text): bot.send_message(chat_id, text) try: tts = gTTS(text=text, lang='ms') tts.save("reply.mp3") audio = open("reply.mp3", 'rb') bot.send_voice(chat_id, audio) audio.close() except: bot.send_message(chat_id, "⚠️ Gagal hantar suara, hanya hantar teks sahaja.")

@bot.message_handler(commands=['start']) def send_welcome(message): welcome = ( "👋 Hai! Saya AI Worldcoin Helper comel anda ✨\n\n" "📷 Hantar gambar wallet, 🎤 mesej suara, atau 💬 taip soalan.\n" "Saya bantu anda claim & cashout Worldcoin ke Hata Wallet 🇲🇾.\n\n" f"🌐 Daftar Worldcoin: 👉 {REFERRAL_LINK}\n" f"💼 Daftar Wallet Hata: 👉 {HATA_WALLET_LINK}" ) bot.reply_to(message, welcome)
...
@bot.message_handler(content_types=['voice']) def handle_voice(message): try: file_info = bot.get_file(message.voice.file_id) downloaded_file = bot.download_file(file_info.file_path)
...
if name == "main": import threading threading.Thread(target=bot.polling).start() app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 5000)))
